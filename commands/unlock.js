...
module.exports = {
    name: 'unlock',
    description: 'Unlocks a channel to a specific role',
    aliases: [''],
    async execute(client, message, args, Discord, server) {
        ...
        if (!args[0]) {
            ...
            if (args[0] == 'server') {
                ...
                message.channel.send('Successfully unlocked server for this role.')
                ...
                message.channel.send('Successfully unlocked ' + args[0] + ' for this role.')
            }
        }
    }
}